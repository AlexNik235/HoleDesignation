namespace Extensions
{
    ...
    public static class ProjectExtensions
    {
            ...
            Options options)
        {
            ...
            var types = GetAssemblyTypes(file, new[] { Constants.Cmd, Constants.AppCmd });
            ...
            {
                types.AddRange(GetAssemblyTypes(f, new[] { Constants.AppCmd }));
            }
            ...
            string[] typeNames)
        {
            var types = Scan(file)
                .Where(x =>
                {
                    ...
                    Console.Write($"\nBaseTypeName - {x.FullName} - types: {string.Join(';', typeNames)}");
                    return typeNames.Any(tm => x.FullName.Contains(tm) || tm.Contains(x.FullName));
                })
                .ToList();
